...
public class BoardTest {
    ...
    @Test
    public void testSimpleGameRedIsComputer(){
        
        System.out.println("Simple Game Red Player is Computer");
        SimpleComputerGame instance = new SimpleComputerGame();
        instance.setBoardSize(6);
        instance.initializeGame();
        
        instance.setComputerPlayer(0);
        for(int i = 0; i < 36; i++){
            for(int j = 0; j < 36; j++){
                if(instance.getRedLetter() == 'S'){
                    instance.setRedLetter('O');
                }else{
                    instance.setRedLetter('S');
                }
                if(instance.makeMove(i, i)==1){
                    break;
                }
            }    
            
            int movei = instance.makeMove(0, 0);
            int lastMove[] = instance.getPeekHistory();
            
            if(instance.isGameOver(instance.isSOS(lastMove[0],lastMove[1])) == 1){
                break;
            }
        } 
        if(instance.getWinner() == 1){
            int expResult = 1;
            assertEquals(expResult,instance.getWinner());
         
        }else if(instance.getWinner() == 0){
            int expResult = 0;
            assertEquals(expResult,instance.getWinner());
        }
    }
    @Test
    public void testGeneralGameBlueIsComputer(){
        
        System.out.println("General Game Blue Player is Computer");
        GeneralComputerGame instance = new GeneralComputerGame();
        instance.setBoardSize(6);
        instance.initializeGame();
        int expResult;
        instance.setComputerPlayer(1);
    ...
    @Test
    public void testGeneralBothComputer(){
        System.out.println("General Game Both are Computer");
        GeneralComputerGame instance = new GeneralComputerGame();
        instance.setBoardSize(6);
        instance.initializeGame();
        instance.setComputerPlayer(2);
    ...
    @Test
    public void testSimpleGameBothComputer(){
        System.out.println("Simple Game Both are Computer");
        SimpleComputerGame instance = new SimpleComputerGame();
        instance.setBoardSize(6);
        instance.initializeGame();
